...
class AnexoController extends Controller
{
    ...
    public function getLogosImpresion(Request $request)
    {
        $codGrupoCia = $request->input('codGrupoCia');
        $codLocal = $request->input('codLocal');
        ...
        try {
            $conn = OracleDB::getConnection();
            $cursor = oci_new_cursor($conn);
            $stid = oci_parse($conn, 'begin :result := HHC_LOOK_AND_FEEL.F_GET_LOGOS_HHC(
                cCodCia => :cCodCia,
                cCodGrupoCia => :cCodGrupoCia);end;');
            oci_bind_by_name($stid, ':result', $cursor, -1, OCI_B_CURSOR);
            oci_bind_by_name($stid, ':cCodGrupoCia', $codGrupoCia);
            oci_bind_by_name($stid, ':cCodCia', $codLocal);
            oci_execute($stid);
            oci_execute($cursor);
            ...
            return CustomResponse::success('Logos para impresión', $lista);
        } catch (\Throwable $th) {
            return CustomResponse::failure($th->getMessage());
        }
    }
